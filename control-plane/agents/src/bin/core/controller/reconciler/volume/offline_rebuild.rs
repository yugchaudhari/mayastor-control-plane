...
async fn offline_rebuild_reconcile(
    ...
    // Only operate on unpublished volumes; published volumes are handled by
    // the existing HotSpareReconciler.
    if volume.as_ref().target().is_some() {
        return PollResult::Ok(PollerState::Idle);
    }
    ...
    if volume_state.status == VolumeStatus::Degraded {
        tracing::info!(
            ...
            "Unpublished volume is Degraded; eligible for offline rebuild (detection-only in Iteration 1)"
        );
    }
